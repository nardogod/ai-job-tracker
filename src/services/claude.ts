...
export class ClaudeService {
  ...
  async analyzeMatch(profile: Profile, job: Job): Promise<MatchScore> {
    ...
    try {
      ...
      const analysis = await this.callClaudeWithRetry(profile, job);
  ...
  private async callClaudeWithRetry(
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
      try {
        const analysis = await this.getStructuredAnalysis(profile, job);
        if (attempt > 0) {
   ...
   * @private
   */
  private async getStructuredAnalysis(profile: Profile, job: Job): Promise<ClaudeResponse> {
